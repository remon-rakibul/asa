...
from tools.database import get_conversation, list_conversations, log_turn
...
async def conversations(clinic_id: int = Depends(current_clinic_id)):
    return await list_conversations(clinic_id)


@router.get("/{session_id}", response_model=list[ConversationMessage])
async def conversation(session_id: str, clinic_id: int = Depends(current_clinic_id)):
...
async def draft_reply(
    session_id: str, clinic_id: int = Depends(current_clinic_id),
) -> dict:
    ...
    """
    transcript = await get_conversation(clinic_id, session_id)
    if not transcript:
...
async def reply(
    ...
    provider. Web-portal threads (`pt-acc…`) get the message appended to the
    LangGraph thread, so the patient sees it in the chat (and the agent sees
    it in its history); voice sessions have no outbound text path.
    """
    ...
    # Record the staff message so it shows in the transcript.
    await log_turn(
        clinic_id=clinic_id, session_id=session_id, channel=channel,
        role="assistant", text=text, channel_identifier=number,
    )
    return {"ok": True}
