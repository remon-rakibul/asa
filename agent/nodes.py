...
from . import prompts
from .memory import build_visit_record, write_patient_memory
from .state import AppointmentState
from .tools import BOOKING_TOOLS, MANAGE_TOOLS, SEARCH_TOOLS, search_hospital_info
...
def _tools_for(rag: bool, manage: bool, search: bool) -> list:
    """Tool list sized to the session — shared by turn/prewarm/suggest so all
    three render the identical schema (prefix-cache match)."""
...
@lru_cache
def _llm_bound(rag: bool, manage: bool, search: bool = False):
    ...
    when the patient is authenticated, the cross-hospital search only on
    platform-mode threads.
    """
    return _build_chat_model().bind_tools(_tools_for(rag, manage, search))


@lru_cache
def _llm_prewarm(rag: bool, manage: bool, search: bool = False):
...
@lru_cache
def _llm_suggest(rag: bool, manage: bool, search: bool = False):
...
def _binding_flags(state: AppointmentState) -> tuple[bool, bool, bool]:
    """(rag, manage, search) for this thread — the single source of truth so
    turn, prewarm, and suggest bindings can never drift apart.

    Platform mode binds search_doctors AND search_hospital_info, both
    permanently — the binding must not flip when choose_doctor lands the
    thread on a hospital mid-life (KV cache + lru_cache identity). Unlike
    non-platform mode, RAG is not conditioned on "this hospital has docs":
    search_hospital_info resolves its own scope from state at CALL time
    (falling back to a cross-hospital search when no hospital is chosen
    yet), so the tool is useful even before any hospital has docs.
    """
    if state.get("platform_mode"):
        return True, bool(state.get("patient_account_id")), True
    hospital_id = state.get("hospital_id")
    rag = bool(hospital_id and _hospital_has_docs_sync(hospital_id))
    return rag, bool(state.get("patient_account_id")), False
...
def _llm_for(hospital_id: int | None, account_id: int | None = None,
    ...
    if platform:
        return _llm_bound(rag=True, manage=bool(account_id), search=True)
    rag = bool(hospital_id and _hospital_has_docs_sync(hospital_id))
...
async def _write_patient_memory(state: AppointmentState) -> None:
    """Persist patient profile + booking visit after a successful agent booking.

    Thin wrapper over agent.memory.write_patient_memory (shared with the
    direct portal-booking endpoint). No-op when the store is unavailable or
    the patient is not authenticated.
    """
    store = get_store()
    ...
    await write_patient_memory(
        ...
        phone=state.get("patient_mobile"),
        appointment_id=appointment_id,
        visit=visit,
    )


# Fixed, thread-life-static rag_context for platform-mode threads (see
# _binding_flags — search_hospital_info is bound permanently there). If no
# hospital is chosen yet, the tool itself falls back to searching across
# ALL hospitals (tools/rag.py::search_docs, hospital_id=None) — this
# instruction covers that case so the model attributes each answer to the
# hospital it came from.
_PLATFORM_RAG_CONTEXT = (
    "Information queries: If the patient asks about a hospital's policies, visiting "
    "hours, fees, directions, a specific PERSON or NAME ('কে?', 'who is …?'), or any "
    "factual question that is NOT about booking an appointment, you MUST call "
    ...
    "deflect to booking. If no hospital is chosen yet, the search covers ALL "
    "hospitals on the platform; each result names its hospital — attribute your "
    "answer to that hospital (e.g. mention which hospital it's about) rather than "
    "speaking as if it's universal. Only if it returns NO_INFO, politely say that "
    "information isn't available and offer to help with booking an appointment instead."
)
...
async def _prompt_context(state: AppointmentState, cfg: dict, store=None) -> dict:
    ...
    if state.get("platform_mode"):
        # PLATFORM MODE — marketplace-home assistant serving every hospital.
        # Static per thread (KV cache): the flag is never cleared, so this
        # section — and the platform branding — stay identical even after
        ...
        ctx["ivr_context"] = (
            ...
            "search_doctors(query) with their specialty or need. Recommend ONLY "
            "from its results — they include hospital, fees (৳), rating, and the "
            "next free time; NEVER invent doctors, fees, or hospitals. The moment "
            "a DOCTORS tool result arrives, IMMEDIATELY present those doctors to "
            "the patient in Bangla — each one's name, hospital, fee and next free "
            "time — and ask which they prefer. Do not ask permission to show the "
            "list, do not greet again, and never leave the reply empty. After the "
            "patient picks one, call choose_doctor(doctor_number), then continue "
            "the normal booking steps. Do NOT call select_department. If "
            "get_available_slots returns NO_SLOTS_AVAILABLE with "
            "ALTERNATIVE_DOCTORS, tell the patient their chosen doctor has no "
            "free time and offer those doctors (name, hospital, fee, next free "
            "time); when they pick one, call choose_doctor(doctor_number)."
        )
    elif hospital_id and not clinic_id:
        if not departments:
    ...
    if clinic_id and not selected_doctor_id:
        ...
        if doctor and doctor.get("name"):
            ...
            fee_new, fee_followup = doctor.get("fee_new"), doctor.get("fee_followup")
            if fee_new is not None or fee_followup is not None:
                fee_bits = []
                if fee_new is not None:
                    fee_bits.append(f"নতুন রোগী ৳{fee_new}")
                if fee_followup is not None:
                    fee_bits.append(f"ফলো-আপ ৳{fee_followup}")
                profile_section += (
                    f"\nVisit fee: {', '.join(fee_bits)} — mention it naturally in "
                    "your own words if fees come up; never invent other amounts."
                )
            doctor_context = (
                f"PRE-SELECTED DOCTOR — the patient chose {identity}{where} "
                "in the portal. That choice IS their intent to book — treat it "
                "exactly like the patient having already said 'I want an "
                "appointment': do not wait for them to state it again, do not "
                "open with a generic 'how can I help' greeting. When the patient "
                "(re)opens the chat after choosing this doctor, open by "
                f"naturally acknowledging you'll help them book with ডা. {doctor['name']}"
                ", then move straight to the next booking step — in your own "
    ...
    # RAG context. Platform mode: search_hospital_info is bound PERMANENTLY
    # (see _binding_flags) so this instruction must be equally permanent and
    # NOT depend on hospital_id/has_docs — that would toggle the prompt's
    # cached head mid-thread when choose_doctor lands on a hospital, and
    # could instruct a call to a tool the (has-docs-gated) binding never
    # bound. Non-platform mode keeps the existing has-docs gate (avoids
    # wasting tokens on a hospital with nothing to search).
    if state.get("platform_mode"):
        ctx["rag_context"] = _PLATFORM_RAG_CONTEXT
    elif hospital_id and _hospital_has_docs_sync(hospital_id):
        ctx["rag_context"] = (
...
async def prewarm_turn(state: AppointmentState) -> None:
    ...
    try:
        ...
        rag, manage, search = _binding_flags(state)
        llm = _llm_prewarm(rag=rag, manage=manage, search=search)
...
async def compose_suggestions(state: AppointmentState, store=None) -> list[str]:
    ...
    try:
        ...
        rag, manage, search = _binding_flags(state)
        llm = _llm_suggest(rag=rag, manage=manage, search=search)
...
def _force_rag_lookup(response, state: AppointmentState, hospital_id: int | None):
    ...
    Platform mode: search_hospital_info is bound PERMANENTLY (_binding_flags),
    so this fires regardless of has_docs — the tool itself degrades to
    NO_INFO, or falls back to a cross-hospital search when no hospital is
    chosen yet. Non-platform mode keeps the has-docs gate: forcing a call to
    a tool that isn't bound there (rag=False when the hospital has no docs)
    would inject a tool_call the LLM was never given a schema for.
    """
    if getattr(response, "tool_calls", None):
        return response
    msgs = state.get("messages") or []
    if not msgs or getattr(msgs[-1], "type", None) != "human":
        return response
    platform = bool(state.get("platform_mode"))
    if not platform and (not hospital_id or not _hospital_has_docs_sync(hospital_id)):
        return response
    query = _last_human_message(state)
...
def _force_doctor_lookup(response, state: AppointmentState):
    ...
    if state.get("available_doctors"):
        return response
    platform = bool(state.get("platform_mode")) and not state.get("clinic_id")
    if not platform and not state.get("clinic_id"):
        return response
    msgs = state.get("messages") or []
    if not msgs or getattr(msgs[-1], "type", None) != "human":
    ...
    if not isinstance(content, str) or not _DOCTOR_LIST_RE.search(content):
        return response
    # Platform threads with no clinic yet have no list_doctors scope — the
    # real doctor data comes from the cross-hospital search instead.
    tool = "search_doctors" if platform else "list_doctors"
    args = {"query": _last_human_message(state) or ""} if platform else {}
    log.info("model fabricated a doctor list without tool call — forcing %s", tool)
    response.content = ""
    response.tool_calls = [{
        "name": tool,
        "args": args,
...
# The PATIENT asking for a doctor/specialist/fees in platform mode. Keyed on
# the patient's stable wording, never the model's dodge phrasing (same lesson
# as _INFO_QUESTION_RE / _SLOT_REQUEST_RE): "ভালো কার্ডিওলজিস্ট কে?",
# "চর্মরোগের ডাক্তার দরকার", "ডা. রহিমের ফি কত?".
_DOCTOR_SEEK_RE = re.compile(
    r"ডাক্তার|ডাঃ|ডা\.|বিশেষজ্ঞ|সার্জন|লজিস্ট|কার্ডিও|চর্ম|শিশু|গাইনি|অর্থো|"
    r"নিউরো|মেডিসিন|ফি\b|খরচ|doctor|specialist|surgeon|cardiolog|dermatolog|"
    r"pediatric|gynecolog|neurolog|orthopedic|fee",
    re.IGNORECASE,
)


def _force_doctor_search(response, state: AppointmentState):
    """Platform mode: the patient asked for a doctor/specialist but the model
    answered without calling search_doctors — force the real search.

    Small local models dodge unfamiliar tools with ever-new refusal wording,
    so this keys on the PATIENT's message (stable) and fires only on the first
    hop, before any doctors are on the table.
    """
    if getattr(response, "tool_calls", None):
        return response
    if not state.get("platform_mode") or state.get("available_doctors"):
        return response
    if state.get("doctor_id"):
        # A doctor is already chosen — the booking flow owns the turn now.
        return response
    msgs = state.get("messages") or []
    if not msgs or getattr(msgs[-1], "type", None) != "human":
        return response
    query = _last_human_message(state)
    if not query or not _DOCTOR_SEEK_RE.search(query):
        return response
    log.info("platform doctor request with no tool call — forcing search_doctors")
    # Drop the dodge text from the stored turn (same as _force_doctor_lookup):
    # the next hop presents the real results; a stale "which hospital?"
    # question in history would contradict them on reload.
    response.content = ""
    response.tool_calls = [{
        "name": "search_doctors",
        "args": {"query": query},
...
def _dedupe_tool_calls(response):
    """Drop exact-duplicate tool calls (same name + args) within ONE response.

    Two identical Commands updating a plain state key (available_doctors,
    offered_slots) in the same super-step raise InvalidUpdateError in
    LangGraph — and a duplicated call is never intentional.
    """
    ...
    for tc in tool_calls:
        ...
        if key in seen:
            log.warning("dropping duplicate tool call %s in one response", tc.get("name"))
            continue
        seen.add(key)
        unique.append(tc)
...
async def call_model_node(state: AppointmentState) -> dict:
    ...
    if hospital_id:
        await _refresh_has_docs(hospital_id)
    llm = _llm_for(
        hospital_id, state.get("patient_account_id"),
        platform=bool(state.get("platform_mode")),
    )
    ...
    response = _force_doctor_search(response, state)
